...
/* Windows FreeBSD 4.x doesn't have IOV_MAX exposed. */
#ifndef IOV_MAX
#if defined(__FreeBSD__) || defined(__APPLE__) || defined(WIN32)
# define IOV_MAX 1024
#endif
#endif
#else
 ...
 */
struct thread_stats {
	//std::mutex   mutex;    //can not use mutex there,  mutex can not use operator=
#define X(name) uint64_t    name;
	THREAD_STATS_FIELDS
#ifdef EXTSTORE
...
typedef struct {
	std::thread::id thread_id;        /* unique ID of this thread */
	struct event_base *base;    /* libevent handle this thread uses */
	struct event notify_event;  /* listen event for notify pipe */
	int notify_receive_fd;      /* receiving end of notify pipe */
	int notify_send_fd;         /* sending end of notify pipe */
	struct thread_stats stats;  /* Stats generated by this thread */
	std::shared_ptr<conn_queue<CQ_ITEM>> new_conn_queue; /* queue of new connections to handle */
...
//	char   *ssl_wbuf;
//#endif
  void    *user;  /* Pointer to the l object serving this connection */
} LIBEVENT_THREAD;
//
///**
// * Response objects
...
/*
 *协议地址和套接口信息，在非连接的UDP中，
 *发送者要指定对方地址端口，接受方用于的到
 *数据来源，如果不需要的话可以设置为NULL
